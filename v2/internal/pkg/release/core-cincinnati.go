...
// data.
func GetUpdates(ctx context.Context, c Client, arch string, channel string, version semver.Version, reqVer semver.Version, mirrorMode, graphDataDir string) (Update, Update, []Update, error) {
	var current Update
	var requested Update
	// Prepare parametrized cincinnati query.
	c.SetQueryParams(arch, channel, version.String())

	graph, err := getGraphData(ctx, c, mirrorMode, graphDataDir)
	if err != nil {
...
func CalculateUpgrades(ctx context.Context, c Client, arch, sourceChannel, targetChannel string, startVer, reqVer semver.Version, mirrorMode, graphDataDir string) (Update, Update, []Update, error) {
	if sourceChannel == targetChannel {
		return GetUpdates(ctx, c, arch, targetChannel, startVer, reqVer, mirrorMode, graphDataDir)
	}
	...
	if source.EQ(target) {
		isBlocked, err := handleBlockedEdges(ctx, c, arch, targetChannel, startVer, mirrorMode, graphDataDir)
		if err != nil {
			return Update{}, Update{}, nil, err
		}
		...
		if isBlocked {
			...
			return GetUpdates(ctx, c, arch, targetChannel, reqVer, reqVer, mirrorMode, graphDataDir)
		}
		return GetUpdates(ctx, c, arch, targetChannel, startVer, reqVer, mirrorMode, graphDataDir)
	}
	...
	// edge is hit.
	latest, err := GetChannelMinOrMax(ctx, c, arch, sourceChannel, false, mirrorMode, graphDataDir)
	if err != nil {
		return Update{}, Update{}, nil, fmt.Errorf(ChannelInfo, sourceChannel, err)
	}
	current, _, upgrades, err := GetUpdates(ctx, c, arch, sourceChannel, startVer, latest, mirrorMode, graphDataDir)
	if err != nil {
		return Update{}, Update{}, nil, fmt.Errorf(ChannelInfo, sourceChannel, err)
	}

	requested, newUpgrades, err := calculate(ctx, c, arch, sourceChannel, targetChannel, latest, reqVer, mirrorMode, graphDataDir)
	if err != nil {
		return Update{}, Update{}, nil, err
	}
...
func calculate(ctx context.Context, c Client, arch, sourceChannel, targetChannel string, startVer, reqVer semver.Version, mirrorMode, graphDataDir string) (requested Update, upgrades []Update, err error) {
	...
	if source.EQ(target) {
		...
		targetVer, err = GetChannelMinOrMax(ctx, c, arch, currChannel, false, mirrorMode, graphDataDir)
		if err != nil {
			return requested, upgrades, err
		}
	}

	isBlocked, err := handleBlockedEdges(ctx, c, arch, currChannel, startVer, mirrorMode, graphDataDir)
	if err != nil {
		return requested, upgrades, err
	}
	if isBlocked {
		// If blocked path is found, just return the requested version and any accumulated
		// upgrades to the caller
		_, requested, _, err = GetUpdates(ctx, c, arch, targetChannel, targetVer, targetVer, mirrorMode, graphDataDir)
		//Warnf is 5?
		klog.Warningf("No upgrade path for %s in target channel %s", startVer.String(), targetChannel)
		return requested, upgrades, err
	}
	...
	_, requested, upgrades, err = GetUpdates(ctx, c, arch, currChannel, startVer, targetVer, mirrorMode, graphDataDir)
	if err != nil {
		return requested, upgrades, err
	}
	...
	currRequested, currUpgrades, err := calculate(ctx, c, arch, currChannel, targetChannel, targetVer, reqVer, mirrorMode, graphDataDir)
	if err != nil {
		return requested, upgrades, err
	}
...
func handleBlockedEdges(ctx context.Context, c Client, arch, targetChannel string, startVer semver.Version, mirrorMode, graphDataDir string) (bool, error) {
	chanVersions, err := GetVersions(ctx, c, arch, targetChannel, mirrorMode, graphDataDir)
	if err != nil {
		return true, err
	}
...
func GetChannelMinOrMax(ctx context.Context, c Client, arch string, channel string, min bool, mirrorMode, graphDataDir string) (semver.Version, error) {
	// Prepare parametrized cincinnati query.
...
// upstream Cincinnati stack given architecture and channel.
func GetVersions(ctx context.Context, c Client, arch, channel string, mirrorMode, graphDataDir string) ([]semver.Version, error) {
	// Prepare parametrized cincinnati query.
	c.SetQueryParams(arch, channel, "")
...
func GetUpdatesInRange(ctx context.Context, c Client, channel, arch string, updateRange semver.Range, mirrorMode, graphDataDir string) ([]Update, error) {
	// Prepare parametrized cincinnati query.
	c.SetQueryParams(arch, channel, "")
...
func getGraphData(ctx context.Context, c Client, mirrorMode, graphDataDir string) (graph graph, err error) {
	if mirrorMode == mirror.DiskToMirror {
		graphDataFiles, err := os.ReadDir(graphDataDir)
		if err != nil {
		...
		fileData, err := os.ReadFile(path.Join(graphDataDir, filename))
		if err != nil {
	...
	if err := writeGraphDataToFile(body, *uri, graphDataDir); err != nil {
		return graph, err
	}
